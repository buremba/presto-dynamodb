...
public class TestDynamodbMetadata
{
    private static final String CONNECTOR_ID = "TEST";
    private static final DynamodbTableHandle NUMBERS_TABLE_HANDLE = new DynamodbTableHandle(CONNECTOR_ID, "example", "numbers");
    private DynamodbMetadata metadata;

    @BeforeMethod
    public void setUp()
            throws Exception
    {
        DynamodbClient client = new DynamodbClient(new DynamodbConfig());
        metadata = new DynamodbMetadata(new DynamodbConnectorId(CONNECTOR_ID), client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new DynamodbColumnHandle(CONNECTOR_ID, "text", createUnboundedVarcharType(), 0),
                "value", new DynamodbColumnHandle(CONNECTOR_ID, "value", BIGINT, 1)));

        // unknown table
        try {
            metadata.getColumnHandles(SESSION, new DynamodbTableHandle(CONNECTOR_ID, "unknown", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
        try {
            metadata.getColumnHandles(SESSION, new DynamodbTableHandle(CONNECTOR_ID, "example", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new DynamodbTableHandle(CONNECTOR_ID, "unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new DynamodbTableHandle(CONNECTOR_ID, "example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new DynamodbTableHandle(CONNECTOR_ID, "unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new DynamodbColumnHandle(CONNECTOR_ID, "text", createUnboundedVarcharType(), 0)),
                new ColumnMetadata("text", createUnboundedVarcharType()));
